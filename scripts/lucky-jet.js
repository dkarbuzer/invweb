...
buttonGetSignal.innerText = 
    language === "ru" ? "получить сигнал" :
    language === "hi" ? "सिग्नल प्राप्त करें" :
    language === "tr" ? "sinyal al" :
    language === "es" ? "obtener señal" :
    "get signal";
...
function startCooldown(seconds) {
    ...
    const interval = setInterval(() => {
        remainingTime--;
        buttonGetSignal.innerText = 
            language === "ru" ? `Подождите ${remainingTime} сек` :
            language === "hi" ? `कृपया ${remainingTime} सेकंड प्रतीक्षा करें` :
            language === "tr" ? `${remainingTime} saniye bekleyin` :
            language === "es" ? `Espere ${remainingTime} seg` :
            `Wait ${remainingTime} sec`;
